...
impl<const N: usize> Bint<N> {
    ...
    pub const fn to_be_bytes(self) -> [u8; Self::BYTES] where crate::BUint<{Self::BYTES}>: Sized {
    ...
    pub const fn to_le_bytes(self) -> [u8; Self::BYTES] where crate::BUint<{Self::BYTES}>: Sized {
        let mut bytes = [0; Self::BYTES];
        let digits = self.uint.digits();
        let mut i = 0;
