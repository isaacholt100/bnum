...
- **Strict adherence to Rust integer API**: the API of `Integer` mimcs the API of the Rust standard library's integer types as closely as possible. Effectively every method available on the integer types from `std` is also available on `Integer`, and has the same behaviour.
